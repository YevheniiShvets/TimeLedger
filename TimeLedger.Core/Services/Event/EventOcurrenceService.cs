...
/*
    ...
    var allEvents = eventRepository.GetAll(ownerType, ownerId);
        var occurrences = new List<EventOccurrenceDto>();

        foreach (var e in allEvents)
        {
            if (e.EventType == EventType.OneTime)
            {
                if (e.StartTime.HasValue && e.EndTime.HasValue &&
                    e.EndTime > rangeStart && e.StartTime < rangeEnd)
                {
                    occurrences.Add(MapEvent(e, e.StartTime.Value, e.EndTime.Value));
                }
            }
            else if (e.EventType == EventType.Recurrence)
            {
                if (e.StartTime.HasValue && e.EndTime.HasValue && e.RecurrenceFrequency.HasValue)
                {
                    var recurrenceRule = new RecurrenceRuleDto
                    {
                        RecurrenceFrequency = e.RecurrenceFrequency.Value,
                        RecurrenceInterval = e.RecurrenceInterval ?? 1,
                        RecurrenceValue = e.RecurrenceValue,
                    ...
                    foreach (var (start, end) in generated)
                    {
                        occurrences.Add(MapEvent(e, start, end));
                    }
                }
            }
            else if (e.EventType == EventType.Deadline)
            {
                if (e.DueAt.HasValue && e.DueAt >= rangeStart && e.DueAt <= rangeEnd)
                {
                    occurrences.Add(new EventOccurrenceDto
                    {
                        EventId = e.Id,
                        Title = e.Title,
                        StartTime = e.DueAt.Value,
                        EndTime = e.DueAt.Value.AddSeconds(1),
                        OwnerType = e.OwnerType,
                        OwnerId = e.OwnerId,
                        EventType = e.EventType
                    });
